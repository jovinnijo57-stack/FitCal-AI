      ...
      try {
        ...
        if (error) {
          if (error.message.includes("rate limit")) {
            setErrors({
              otp: "Supabase email rate limit exceeded. Please wait a few minutes or try another email address!",
            });
          } else {
  ...
  return (
    <div className="min-h-dvh bg-background flex flex-col justify-center py-12 relative">
        ...
        <div className="fixed inset-0 bg-background/80 backdrop-blur-md z-50 flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-gradient-card border border-border rounded-3xl p-8 max-w-sm w-full text-center shadow-glow flex flex-col items-center animate-in zoom-in-95 duration-300">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Your premium PulsePeak account is ready. Redirecting you to login...
            </p>
          </div>
        </div>
      )}
      ...
      <div className="mx-auto w-full max-w-md px-6">
        <button
          onClick={() => (step === "otp" ? setStep("form") : window.history.back())}
        ...
        <div className="mt-8 mb-8">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {step === "otp"
              ? `Enter the code sent to ${formData.email} to verify your account.`
          ...
          <>
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div>
                <div
                  className={clsx(
                    "flex items-center gap-2 rounded-2xl border bg-card/80 backdrop-blur-md px-3 py-3.5 transition-all focus-within:ring-2",
                    errors.phone
                      ? "border-destructive focus-within:ring-destructive/30"
                      : "border-border focus-within:ring-primary/50",
                  )}
