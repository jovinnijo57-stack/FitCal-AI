  ...
  useEffect(() => {
    try {
      const userKey = profile.email
        ? `pulsepeak_water_${profile.email}`
        : "pulsepeak_water_history";
  ...
  return (
    <PhoneShell>
      ...
      <div className="mx-5 my-6 flex flex-col items-center justify-center rounded-3xl border border-border bg-gradient-card p-8 shadow-card relative overflow-hidden">
        ...
        <div className="relative flex h-52 w-52 items-center justify-center rounded-full border-4 border-cyan-500/20 bg-card shadow-inner overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center text-center">
            <Droplet
              className="h-8 w-8 text-cyan-400 mb-1 animate-bounce"
        ...
        >
          ...
          <span>
            {pct >= 100
              ? "Amazing! Daily hydration goal reached! 🎉"
              : `Completed: ${Math.round(pct)}% (${profile.waterGoalMl - waterMl}ml left)`}
          </span>
        </div>
      </div>
      ...
      <div className="mx-5 mt-2">
        <p className="font-display text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3 px-1">
          Quick Add
        </p>
      ...
      <div className="mx-5 mt-6 pb-20">
        <div className="mb-3 flex items-center justify-between px-1">
          <span className="font-display text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            Drinking History Log
          </span>
          <div className="relative flex items-center gap-2">
            <label
              htmlFor="history-date"
              className="flex items-center gap-1.5 cursor-pointer text-xs font-semibold text-cyan-500 hover:opacity-80 bg-cyan-500/10 border border-cyan-500/20 px-3 py-1.5 rounded-xl transition shadow-sm"
            >
            ...
            {selectedDate && (
              <button
                ...
                className="text-xs text-muted-foreground hover:text-destructive flex items-center gap-0.5 bg-muted px-2 py-1.5 rounded-xl transition"
              >
        ...
        <div className="space-y-2.5">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Target: {(profile.waterGoalMl / 1000).toFixed(1)}L
                  </p>
                </div>
              </div>
          ...
          {displayHistory.map((h, idx) => {
            ...
            return (
              <div
                key={idx}
                className="flex items-center justify-between rounded-2xl border border-border bg-gradient-card p-4 shadow-sm"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Target: {(h.goal / 1000).toFixed(1)}L
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-display text-base font-bold text-foreground">
                    {(h.current / 1000).toFixed(1)}L
                  </p>
                  <p
                    className={
                      hPct >= 100
                        ? "text-[10px] text-success font-semibold uppercase tracking-wider mt-0.5"
